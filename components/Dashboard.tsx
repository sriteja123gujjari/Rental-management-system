...
const Dashboard = ({ user, onLogout }: { user: any, onLogout: () => void }) => {
  ...
  // Watch members array changes to auto-select dynamic dropdown inputs properly
  useEffect(() => {
    if (members.length > 0) {
      setNewExpensePayer(prev => prev || members[0]);
      setPaymentCollector(prev => prev || members[0]);
    }
  }, [members]);
  ...
  const refreshData = async (silent = false) => {
    ...
    try {
      ...
      // Fallback workspace default for completely empty profiles
      setMembers(dbMembers.length > 0 ? dbMembers : ['Admin']);
    } catch (err) {
  ...
  const monthlyData = useMemo(() => {
    ...
    const share = net / totalMembersCount;
    ...
    const activeSplit = activeNet / totalMembersCount;
  ...
  const handleSaveShop = async () => { 
    ...
    try { 
      if (editingShopId) { 
        ...
        await api.addShop(currentMonth, { name: newShopName, baseRent: Number(newShopRent) }, familyId); 
      } 
  ...
  const addExpense = async () => { 
    ...
    setSubmitting(true); 
    try { 
      await api.addExpense(currentMonth, { description: newExpenseDesc, amount: Number(newExpenseAmount), paidBy: newExpensePayer }, familyId); 
      setNewExpenseDesc(''); setNewExpenseAmount(''); setIsCustomExpense(false); await refreshData(true); 
  ...
  const handleAddMember = async (e: React.FormEvent) => {
    ...
    if (newMemberName.trim() && !members.includes(newMemberName.trim())) {
      setSubmitting(true);
      try {
        await api.addMember(newMemberName.trim(), familyId);
        setNewMemberName('');
        await refreshData(true);
      } catch (err) {
        console.error("Could not add custom member", err);
      } finally {
        setSubmitting(false);
      }
    }
  ...
  const handleRemoveMember = async (nameToRemove: string) => {
    if (members.length > 1) {
      try {
        await api.deleteMember(nameToRemove, familyId);
        await refreshData(true);
      } catch (err) {
        console.error("Could not remove member", err);
      }
    } else {
      alert("You need at least one running member to build balance statements.");
    }
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center items-end sm:items-center bg-slate-900/60 p-0 sm:p-4 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in slide-in-from-bottom-10 sm:zoom-in duration-300">
            ...
            <div className="p-6 space-y-6">
                ...
                <div><label className="block text-xs font-bold text-slate-500 uppercase mb-2 tracking-wider">Collected By</label><div className="flex flex-wrap gap-2 bg-slate-100/50 p-1.5 rounded-2xl">{members.map(m => (<button key={m} onClick={() => setPaymentCollector(m)} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-sm ${paymentCollector === m ? 'bg-white text-indigo-600 shadow-md ring-1 ring-black/5 scale-[1.02]' : 'text-slate-400 hover:text-slate-600 hover:bg-white/50'}`}>{m}</button>))}</div></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
                ...
                <div className="relative z-10">
                    ...
                    <div className="grid gap-3">
                        {monthlyData.transactions.length > 0 ? (
                            ...
                            <p className="text-center py-4 text-slate-400 text-sm font-medium italic">Everything is currently balanced!</p>
                        )}
                    </div>
                </div>
            </section>
            ...
            <section className="bg-white/80 backdrop-blur-xl rounded-[2.5rem] shadow-xl shadow-indigo-100/50 border border-white/20 overflow-hidden ring-1 ring-slate-100">
               ...
               {shops.length === 0 ? (
                  <div className="p-12 text-center flex flex-col items-center justify-center">
                     ...
                     <h3 className="font-bold text-slate-700">No properties built yet!</h3>
                     <p className="text-xs text-slate-400 max-w-sm mt-1"> Add a property configuration below to begin tracking real-time asset flows.</p>
                  </div>
               ) : (
                  <>
                           ...
                           return ( 
                              <div key={shop.id} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col gap-4 relative"> 
                                 <div className="flex justify-between items-start"> 
                                    <div><h3 className="font-bold text-slate-900 text-lg">{shop.name}</h3><p className="text-sm text-slate-400 font-mono mt-0.5">Rent: ₹{formatCurrency(shop.base_rent)}</p></div> 
                                    <div className="flex gap-2 items-center"> 
                                       {isPaid && (<div className="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider">Paid</div>)} 
                                 ...
                                 <div className="bg-slate-50/80 p-4 rounded-xl space-y-4"> 
                                    <div className="flex justify-between items-center border-b border-slate-200 pb-4"> 
                                       <div className="flex flex-col"><span className="text-[10px] uppercase text-slate-400 font-bold tracking-wider">ARREARS</span> {pastDue > 0 ? (<span className="text-rose-600 font-black text-lg mt-1">₹{formatCurrency(pastDue)}</span>) : (<span className="text-emerald-600 font-bold text-sm mt-1 flex items-center gap-1"><CheckCircle2 size={12}/> Clear</span>)} </div> 
                                       <div className="flex flex-col items-end"><span className="text-[10px] uppercase text-slate-400 font-bold tracking-wider">DUE</span><span className={`font-bold text-lg ${outstandingBalance > 0 ? 'text-rose-600' : 'text-emerald-600'}`}>₹{formatCurrency(outstandingBalance)}</span></div> 
                                    </div> 
                                    <div className="flex justify-between items-center"><span className="text-[10px] uppercase text-emerald-600 font-bold tracking-wider">PAID</span><span className="text-emerald-700 font-bold text-lg">₹{formatCurrency(paidAmount)}</span></div> 
                                 </div> 
                                 <div> 
                                    {!isPaid ? (
                                       <button onClick={() => handleOpenPaymentModal(shop, record, outstandingBalance)} className="w-full py-3.5 bg-slate-900 text-white rounded-xl font-bold text-sm shadow-lg flex items-center justify-center gap-2 hover:bg-black"> <CreditCard size={18}/> Record Payment </button>
                                    ) : (
                                       <div className="flex items-center gap-2">
                                          <div className="flex-grow bg-emerald-50 border border-emerald-100 p-3 rounded-xl flex items-center justify-between px-4">
                                             <div className="flex flex-col"><span className="text-[10px] font-bold text-emerald-600 uppercase tracking-wide">PAID</span><span className="text-[11px] text-slate-400">to {record?.collected_by}</span></div>
                                             <div className="text-emerald-700 font-bold font-mono text-xl">₹{formatCurrency(record.amount_paid)}</div>
                                          </div>
                                          <button onClick={() => handleOpenPaymentModal(shop, record, outstandingBalance)} className="h-full px-4 bg-white border border-slate-200 rounded-xl text-slate-400 hover:text-indigo-500 transition-all"><Pencil size={18}/></button>
                                          <button onClick={() => clearPayment(shop.id)} className="h-full px-4 bg-white border border-rose-100 text-rose-400 hover:text-rose-600 transition-all"><X size={18}/></button>
                                       </div>
                                    )} 
                                 </div> 
                              </div> 
                        ...
                        <table className="w-full text-left min-w-[700px]"> 
                           <thead> 
                              <tr className="bg-slate-50 text-xs font-bold uppercase tracking-wider text-slate-400 border-b border-slate-100"> 
                                 ...
                                 <th className="px-6 py-5 text-rose-700">Remaining Due</th> 
                              ...
                              {shops.map((shop) => { 
                                 ...
                                 return ( 
                                    <tr key={shop.id} className="hover:bg-slate-50 transition-colors"> 
                                       <td className="px-8 py-5 font-bold text-slate-700 text-sm">{shop.name}</td> 
                                       <td className="px-6 py-5 font-mono text-xs text-slate-500">₹{formatCurrency(shop.base_rent)}</td> 
                                       <td className="px-6 py-5 font-mono text-xs font-bold">{pastDue > 0 ? (<span className="text-rose-600">₹{formatCurrency(pastDue)}</span>) : (<span className="text-emerald-500 opacity-80 flex items-center gap-1"><CheckCircle2 size={10}/> Clear</span>)}</td> 
                                       <td className="px-6 py-5 font-mono text-sm font-bold"><span className={outstandingBalance > 0 ? 'text-rose-600' : 'text-emerald-600'}>₹{formatCurrency(outstandingBalance)}</span></td> 
                                       <td className="px-6 py-5"> 
                                          <div className="flex flex-col gap-2 max-w-[240px] mx-auto"> 
                                             {!isPaid ? (
                                                <button onClick={() => handleOpenPaymentModal(shop, null, outstandingBalance)} disabled={isProcessing} className="flex items-center justify-center gap-2 w-full py-2 bg-slate-900 text-white rounded-xl text-xs font-bold hover:bg-black transition-all shadow-md"> {isProcessing ? <Loader2 size={12} className="animate-spin"/> : <CreditCard size={12} />} Record Payment </button>
                                             ) : (
                                                <div className="flex items-center gap-2">
                                                   <div className="flex-1 bg-emerald-50 border border-emerald-100 rounded-xl p-2 flex flex-col items-center"><span className="text-[9px] text-emerald-600 font-bold uppercase tracking-wide">PAID</span><span className="text-sm font-bold text-emerald-700 font-mono">₹{formatCurrency(record?.amount_paid)}</span><span className="text-[9px] text-slate-400 mt-0.5">{record?.collected_by}</span></div>
                                                   <div className="flex flex-col gap-1"><button onClick={() => handleOpenPaymentModal(shop, record, outstandingBalance)} className="p-1.5 bg-white border border-slate-200 rounded-lg hover:bg-indigo-50 hover:text-indigo-600 text-slate-400 transition-colors"> <Pencil size={14} /> </button><button onClick={() => clearPayment(shop.id)} className="p-1.5 bg-white border border-slate-200 rounded-lg hover:bg-rose-50 hover:text-rose-600 text-slate-400 transition-colors"> <X size={14} /> </button></div>
                                                </div>
                                             )} 
                                          </div> 
                                       </td> 
               ...
               <div className="p-6 bg-slate-50 border-t border-slate-100"> 
                  <div className="flex flex-col sm:flex-row gap-3"> 
                     <div className="w-full"> 
                        <input type="text" placeholder="Shop Name" className="w-full px-5 h-14 rounded-2xl border border-slate-200 focus:border-indigo-500 outline-none text-sm font-bold shadow-sm" value={newShopName} onChange={(e) => setNewShopName(e.target.value)} /> 
                     </div> 
                     <div className="flex gap-3 w-full sm:w-auto"> 
                        <div className="flex-grow sm:w-40"> 
                           <input type="number" placeholder="Rent" className="w-full px-5 h-14 rounded-2xl border border-slate-200 focus:border-indigo-500 outline-none text-sm font-bold shadow-sm" value={newShopRent} onChange={(e) => setNewShopRent(e.target.value)} /> 
                        </div> 
                        <button onClick={handleSaveShop} disabled={submitting || !newShopName || !newShopRent} className="h-14 px-8 bg-slate-900 text-white rounded-2xl font-bold hover:bg-black transition-all flex items-center justify-center gap-2 text-sm shadow-xl disabled:opacity-70 whitespace-nowrap"> 
                           {submitting ? <Loader2 size={18} className="animate-spin" /> : (editingShopId ? <Save size={18} /> : <Plus size={18} />)} 
                           {editingShopId ? 'Update' : 'Add Shop'} 
                        </button> 
                        {editingShopId && (<button onClick={() => { setEditingShopId(null); setNewShopName(''); setNewShopRent(''); }} className="h-14 w-14 flex items-center justify-center bg-white border border-slate-200 text-slate-400 rounded-2xl hover:text-rose-500"> <X size={20} /> </button>)} 
                     </div> 
                  </div> 
               </div>
            </section>
            ...
            <section className="bg-white rounded-[2.5rem] shadow-2xl p-6 sm:p-8 border border-slate-100 relative overflow-hidden group"> 
              <h3 className="text-lg font-bold mb-6 flex items-center gap-3 text-slate-800 relative z-10"> <div className="p-2.5 bg-gradient-to-br from-violet-600 to-indigo-600 rounded-2xl text-white shadow-lg"> <CreditCard size={22} /> </div> Quick Expense Record </h3> 
              <div className="flex flex-col gap-6 relative z-10"> 
                   ...
                   <div className="relative group">
                      {isCustomExpense ? (
                         <div className="relative animate-in fade-in zoom-in duration-200">
                            <input type="text" autoFocus placeholder="What did you pay for?" className="w-full pl-5 pr-14 h-16 rounded-2xl bg-slate-50 border-2 border-slate-100 focus:bg-white focus:border-violet-500 outline-none text-slate-700 font-bold text-lg transition-all shadow-sm" value={newExpenseDesc} onChange={(e) => setNewExpenseDesc(e.target.value)} />
                            <button onClick={() => setIsCustomExpense(false)} className="absolute right-3 top-1/2 -translate-y-1/2 p-2 bg-white text-slate-400 hover:text-rose-500 rounded-xl border border-slate-100 shadow-sm"> <List size={20} /> </button>
                         </div>
                      ) : (
                         <div className="relative group/select">
                            <select className="w-full appearance-none pl-5 pr-12 h-16 rounded-2xl bg-slate-50 border-2 border-slate-100 focus:bg-white focus:border-violet-500 outline-none text-slate-700 font-bold text-lg transition-all cursor-pointer shadow-sm" value={newExpenseDesc} onChange={(e) => { if (e.target.value === 'CUSTOM_ENTRY_TRIGGER') { setIsCustomExpense(true); setNewExpenseDesc(''); } else { setNewExpenseDesc(e.target.value); } }}> 
                               <option value="">Select Category</option> 
                               <option value="CUSTOM_ENTRY_TRIGGER" className="font-bold text-violet-600 bg-violet-50">Custom Entry</option>
                               {PREDEFINED_EXPENSES.map(exp => ( <option key={exp} value={exp}>{exp}</option> ))}
                            </select> 
                            <div className="absolute right-5 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400"> <ChevronRight size={20} className="rotate-90" /> </div>
                         </div>
                      )}
                   </div>
                </div> 
                <div className="flex gap-4"> 
                   <div className="w-1/2"> 
                      ...
                      <div className="relative group/amount"><span className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-xl">₹</span><input type="number" placeholder="0" className="w-full pl-10 pr-4 h-16 rounded-2xl bg-slate-50 border-2 border-slate-100 focus:bg-white focus:border-violet-500 outline-none text-slate-800 font-black text-2xl transition-all shadow-sm" value={newExpenseAmount} onChange={(e) => setNewExpenseAmount(e.target.value)} /> </div>
                   </div> 
                   <div className="w-1/2"> 
                      <label className="text-[11px] font-bold text-slate-400 uppercase ml-1 block mb-2.5 tracking-widest">Paid By</label> 
                      <div className="relative group/payer">
                         <select className="w-full appearance-none pl-5 pr-12 h-16 rounded-2xl bg-slate-50 border-2 border-slate-100 focus:bg-white focus:border-violet-500 outline-none text-slate-700 font-bold text-lg transition-all cursor-pointer shadow-sm" value={newExpensePayer} onChange={(e) => setNewExpensePayer(e.target.value)}> 
                            {members.map(m => <option key={m} value={m}>{m}</option>)} 
                         </select> 
                         <div className="absolute right-5 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400"> <UserCircle size={22} /> </div>
                      </div>
                   </div> 
                </div> 
                <button onClick={addExpense} disabled={submitting || !newExpenseDesc || !newExpenseAmount} className="w-full h-16 mt-2 bg-slate-900 text-white rounded-2xl font-bold hover:bg-black transition-all flex items-center justify-center gap-3 text-base shadow-xl disabled:opacity-50"> 
                   {submitting ? <Loader2 size={20} className="animate-spin" /> : <Plus size={20} />} Record Expense 
                </button> 
              </div> 
            </section>
          ...
          <div className="space-y-8">
            {/* DYNAMIC FAMILY GROUP MANAGER */}
            <section className="bg-white rounded-[2rem] shadow-xl p-6 border border-slate-100">
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                 <Users size={16} className="text-indigo-500" /> Group Partners Setup
              </h3>
              
              <form onSubmit={handleAddMember} className="flex gap-2 mb-4">
                <input 
                  type="text" 
                  placeholder="Partner Name (e.g. Srinivas)" 
                  className="flex-grow px-3 py-2 text-xs rounded-xl border border-slate-200 outline-none focus:border-indigo-500"
                  value={newMemberName}
                  onChange={(e) => setNewMemberName(e.target.value)}
                />
                <button type="submit" disabled={submitting} className="px-4 bg-slate-900 text-white rounded-xl hover:bg-black text-xs font-bold transition-all">
                   Add
                </button>
              </form>
              ...
              <div className="flex flex-wrap gap-2">
                {members.map((member, idx) => (
                  <div key={idx} className="flex items-center gap-1.5 bg-slate-100 px-3 py-1.5 rounded-xl text-xs font-bold text-slate-700 border border-slate-200/60">
                    <span>{member}</span>
                    <button type="button" onClick={() => handleRemoveMember(member)} className="text-slate-400 hover:text-rose-600 transition-colors">
            ...
            <div className="bg-amber-50 p-6 rounded-[2rem] border border-amber-100/50 flex gap-4 shadow-sm"> 
              ...
              <div className="space-y-1">
                <h4 className="text-xs font-bold text-amber-600 uppercase tracking-wider">Dynamic Workspace Activation</h4>
                <p className="text-[11px] text-amber-700/80 leading-relaxed font-medium"> 
                  All updates sync in real time. Revenue split profiles dynamically scale matching total registered account management metrics above.
                </p> 
              </div>
            </div>

            <div className="pb-8">
              <button onClick={handleShare} className="w-full flex items-center justify-center gap-3 px-6 py-5 bg-emerald-600 text-white rounded-3xl font-bold hover:bg-emerald-700 transition-all shadow-xl text-base"> 
                <Share2 size={22} /> 
...
const StatCard = ({ title, value, type, icon: Icon }: any) => {
  ...
  return (
    <div className={`bg-white p-5 rounded-3xl border ${style.border} shadow-lg shadow-slate-200/40 flex flex-col justify-between min-h-[110px] transition-all hover:-translate-y-1`}> 
       <div className="flex justify-between items-start mb-2"> 
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{title}</p> 
          <div className={`p-2 rounded-xl ${style.iconBg} ${style.iconColor}`}><Icon size={16} /></div> 
       </div> 
       <h3 className={`text-2xl font-bold tracking-tighter ${style.textColor}`}>₹{formatCurrency(Math.abs(value))}</h3> 
    </div>
  );
};
