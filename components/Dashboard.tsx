...
const Dashboard = ({ user, onLogout }: { user: any, onLogout: () => void }) => {
  ...
  const handleSettlePayment = async (toMember: string, amount: number) => {
    ...
    if (vpa) {
        try { await navigator.clipboard.writeText(vpa); console.log(`UPI ID Copied: ${vpa}`); } catch (e) {}
        window.location.href = `upi://pay?pa=${vpa}&pn=${toMember}&am=${amount}&cu=INR`;
    } else { console.error("UPI ID not found"); }
  };
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-white rounded-[2.5rem] shadow-2xl shadow-indigo-100/60 p-6 sm:p-8 border border-slate-100 relative overflow-hidden group hover:shadow-indigo-200/50 transition-all duration-500"> 
              ...
              <div className="flex flex-col gap-6 relative z-10"> 
                <div> <label className="text-[11px] font-bold text-slate-400 uppercase ml-1 block mb-2.5 tracking-widest">Description</label> <div className="relative group">{isCustomExpense ? (<div className="relative animate-in fade-in zoom-in duration-200"><input type="text" autoFocus placeholder="What did you pay for?" className="w-full pl-5 pr-14 h-16 rounded-2xl bg-slate-50 border-2 border-slate-100 focus:bg-white focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 outline-none text-slate-700 font-bold text-lg transition-all shadow-sm placeholder:text-slate-300 placeholder:font-medium" style={{ fontFamily: '"Plus Jakarta Sans", sans-serif' }} value={newExpenseDesc} onChange={(e) => setNewExpenseDesc(e.target.value)} onKeyDown={(e) => { if(e.key === 'Escape') setIsCustomExpense(false); }} /><button onClick={() => setIsCustomExpense(false)} className="absolute right-3 top-1/2 -translate-y-1/2 p-2 bg-white text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-xl border border-slate-100 shadow-sm transition-all active:scale-90" title="Switch to List"> <List size={20} /> </button></div>) : (<div className="relative group/select"><select className="w-full appearance-none pl-5 pr-12 h-16 rounded-2xl bg-slate-50 border-2 border-slate-100 focus:bg-white focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 outline-none text-slate-700 font-bold text-lg transition-all cursor-pointer shadow-sm hover:border-violet-200" style={{ fontFamily: '"Plus Jakarta Sans", sans-serif' }} value={newExpenseDesc} onChange={(e) => { if (e.target.value === 'CUSTOM_ENTRY_TRIGGER') { setIsCustomExpense(true); setNewExpenseDesc(''); } else { setNewExpenseDesc(e.target.value); } }}> <option value="">Select Category</option> <option value="CUSTOM_ENTRY_TRIGGER" className="font-bold text-violet-600 bg-violet-50">Custom</option><hr />{PREDEFINED_EXPENSES.map(exp => ( <option key={exp} value={exp} className="text-slate-700 font-medium py-2">{exp}</option> ))}</select> <div className="absolute right-5 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400 group-hover/select:text-violet-500 transition-colors"> <ChevronRight size={20} className="rotate-90" /> </div></div>)}</div></div> 
                <div className="flex gap-4"> <div className="w-1/2"> <label className="text-[11px] font-bold text-slate-400 uppercase ml-1 block mb-2.5 tracking-widest">Amount</label> <div className="relative group/amount"><span className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-xl group-focus-within/amount:text-violet-500 transition-colors">₹</span><input type="number" placeholder="0" className="w-full pl-10 pr-4 h-16 rounded-2xl bg-slate-50 border-2 border-slate-100 focus:bg-white focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 outline-none text-slate-800 font-black text-2xl transition-all shadow-sm placeholder:text-slate-300" style={{ fontFamily: '"Plus Jakarta Sans", sans-serif' }} value={newExpenseAmount} onChange={(e) => setNewExpenseAmount(e.target.value)} /> </div></div> <div className="w-1/2"> <label className="text-[11px] font-bold text-slate-400 uppercase ml-1 block mb-2.5 tracking-widest">Paid By</label> <div className="relative group/payer"><select className="w-full appearance-none pl-5 pr-12 h-16 rounded-2xl bg-slate-50 border-2 border-slate-100 focus:bg-white focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 outline-none text-slate-700 font-bold text-lg transition-all cursor-pointer shadow-sm hover:border-violet-200" style={{ fontFamily: '"Plus Jakarta Sans", sans-serif' }} value={newExpensePayer} onChange={(e) => setNewExpensePayer(e.target.value)}> {MEMBERS.map(m => <option key={m} value={m}>{m}</option>)} </select> <div className="absolute right-5 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400 group-hover/payer:text-violet-500 transition-colors"> <UserCircle size={22} /> </div></div></div> </div> 
                <button onClick={addExpense} disabled={submitting || !newExpenseDesc || !newExpenseAmount} className="w-full h-16 mt-2 bg-slate-900 text-white rounded-2xl font-bold hover:bg-black hover:scale-[1.01] active:scale-[0.98] transition-all flex items-center justify-center gap-3 text-base shadow-xl shadow-slate-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none disabled:shadow-none" style={{ fontFamily: '"Plus Jakarta Sans", sans-serif' }}> {submitting ? <Loader2 size={20} className="animate-spin" /> : <Plus size={20} />} Record Expense </button> 
              </div> 
            </section>
            ...
            <section className="bg-white rounded-[2rem] shadow-xl shadow-slate-200/40 border border-slate-100 overflow-hidden"> 
              ...
              <div className="p-4 space-y-3 custom-scrollbar"> {expenses.length === 0 ? (<p className="text-center py-10 text-slate-400 text-sm font-medium italic">No expenses recorded for this month.</p>) : (expenses.map((exp) => (<div key={exp.id} className="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-100 shadow-sm gap-3 hover:border-indigo-100 transition-colors"> <div className="flex items-center gap-4"> <div className="w-12 h-12 rounded-xl bg-slate-50 text-slate-400 flex items-center justify-center border border-slate-100"><Receipt size={20} /></div> <div> <p className="font-bold text-slate-700 text-sm">{exp.description}</p> <p className="text-[10px] text-slate-400 uppercase tracking-wider font-bold mt-0.5">Paid by <span className="text-indigo-500">{exp.paid_by}</span></p> </div> </div> <div className="flex items-center gap-4"> <span className="font-bold text-rose-500 text-lg font-mono">-₹{formatCurrency(exp.amount)}</span> <button onClick={() => deleteExpense(exp.id)} disabled={processingId === exp.id} className="p-2 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-colors"> {processingId === exp.id ? <Loader2 size={18} className="animate-spin text-rose-500" /> : <Trash2 size={18} />} </button> </div> </div>)))} </div> 
            </section>
          </div>
