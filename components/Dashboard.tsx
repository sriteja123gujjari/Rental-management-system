...
import { 
  ...
  Loader2, ArrowRightLeft, UserCircle, Save, AlertCircle, Pencil, X, LogOut, Share2, List, CheckCircle2
} from 'lucide-react';
...
// UPI IDs for Payment Features (Web Version)
...
const Dashboard = ({ user, onLogout }: { user: any, onLogout: () => void }) => {
  // ✅ GET FAMILY ID from User Metadata (Defaults to Gujjari if missing)
  ...
  // ✅ Web-Only Settlement Handler (Replaces Capacitor)
  const handleSettlePayment = async (toMember: string, amount: number) => {
    ...
    // 1. Copy to Clipboard (Web Standard)
    try {
    ...
    // 2. Try to open UPI link (Works on Mobile Web)
    const upiUrl = `upi://pay?pa=${vpa}&pn=${toMember}&am=${amount}&cu=INR`;
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-slate-800 font-sans pb-24 pt-8 md:pt-0 relative selection:bg-indigo-100 selection:text-indigo-900">
      ...
      {paymentModalOpen && selectedShop && (
        <div className="fixed inset-0 z-50 flex items-center sm:items-center justify-center items-end bg-slate-900/60 p-0 sm:p-4 backdrop-blur-sm animate-in fade-in duration-200">
            ...
            <div className="p-6 space-y-6">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                    <span className="block text-slate-400 font-bold text-[10px] uppercase tracking-wider">Base Rent</span>
                    <span className="font-mono font-bold text-slate-700 text-lg">₹{formatCurrency(selectedShop.base_rent)}</span>
                  </div>
                  <div className="bg-rose-50 p-4 rounded-2xl border border-rose-100">
                    <span className="block text-rose-400 font-bold text-[10px] uppercase tracking-wider">Arrears</span>
                    <span className="font-mono font-bold text-rose-600 text-lg">₹{formatCurrency(arrears[selectedShop.name] || 0)}</span>
                  </div>
                </div>
                
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-2 tracking-wider">Amount Collected</label>
                  <div className="relative group">
                    ...
                    <input 
                      type="number" 
                      className="w-full pl-10 pr-4 h-16 rounded-2xl bg-slate-50 border-2 border-transparent focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none font-bold text-2xl text-slate-800 transition-all shadow-sm placeholder:text-slate-300"
                      value={paymentAmount}
                  ...
                  <div className="flex justify-between items-center mt-2 px-1">
                     <span className="text-[10px] font-bold text-slate-400 uppercase">Remaining Due</span>
                     <span className="text-sm font-black text-slate-700">
                       ₹{formatCurrency((selectedShop.base_rent + (arrears[selectedShop.name] || 0)) - (Number(paymentAmount) || 0))}
                     </span>
                  </div>
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-2 tracking-wider">Collected By</label>
                  <div className="flex gap-2 bg-slate-100/50 p-1.5 rounded-2xl">
                    {MEMBERS.map(m => (
                      <button
                        key={m}
                        onClick={() => setPaymentCollector(m)}
                        className={`flex-1 py-3 rounded-xl text-xs font-bold transition-all shadow-sm ${paymentCollector === m ? 'bg-white text-indigo-600 shadow-md ring-1 ring-black/5 scale-[1.02]' : 'text-slate-400 hover:text-slate-600 hover:bg-white/50'}`}
                      >
                        {m}
                      </button>
                    ))}
                ...
                <div className="pt-2 flex gap-3">
                  <button onClick={() => setPaymentModalOpen(false)} className="flex-1 h-14 rounded-2xl font-bold text-slate-500 hover:bg-slate-100 transition-colors">Cancel</button>
                  <button 
                    onClick={submitPayment} 
                    disabled={submitting}
                    className="flex-[2] h-14 bg-slate-900 text-white rounded-2xl font-bold hover:bg-black transition-all shadow-xl shadow-slate-200 disabled:opacity-70 flex items-center justify-center gap-2 active:scale-95"
                  >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            {monthlyData.transactions.length > 0 && (
              <section className="bg-slate-900 rounded-[2rem] shadow-2xl shadow-indigo-900/20 p-6 sm:p-8 text-white relative overflow-hidden">
                <div className="absolute top-0 right-0 p-10 opacity-5 pointer-events-none"><ArrowRightLeft size={180} /></div>
                <div className="relative z-10">
                    <h2 className="text-sm font-bold flex items-center gap-2 mb-6 uppercase tracking-widest text-slate-400">
                        <span className="p-1 bg-indigo-500/20 rounded-lg"><ArrowRightLeft size={14} className="text-indigo-400" /></span>
                        Settlement Plan
                    </h2>
                    <div className="grid gap-3">
                      {monthlyData.transactions.map((t: any, idx) => (
                        // ✅ Web-Safe Click Handler
            ...
            <section className="bg-white/80 backdrop-blur-xl rounded-[2.5rem] shadow-xl shadow-indigo-100/50 border border-white/20 overflow-hidden ring-1 ring-slate-100">
               <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-white/50"> 
                  <h2 className="text-lg font-black flex items-center gap-3 text-slate-800">
                    <div className="p-2 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl shadow-lg shadow-indigo-200 text-white"><Building2 size={20} /></div>
                    Shop Status
                  </h2> 
                           ...
                           <div className="flex justify-between items-center border-b border-slate-200 pb-4">
                              <div className="flex flex-col">
                                ...
                                {pastDue > 0 ? (
                                  ...
                                  <span className="text-emerald-600 font-bold text-sm mt-1 flex items-center gap-1"><CheckCircle2 size={12}/> Clear</span>
                                )}
                              </div>
               ...
               <div className="p-6 bg-slate-50 border-t border-slate-100"> 
                   <div className="flex flex-col sm:flex-row gap-3"> <div className="w-full"> <input type="text" placeholder="Shop Name" className="w-full px-5 h-14 rounded-2xl border border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none text-sm font-bold shadow-sm" value={newShopName} onChange={(e) => setNewShopName(e.target.value)} /> </div> <div className="flex gap-3 w-full sm:w-auto"> <div className="flex-grow sm:w-40"> <input type="number" placeholder="Rent" className="w-full px-5 h-14 rounded-2xl border border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none text-sm font-bold shadow-sm" value={newShopRent} onChange={(e) => setNewShopRent(e.target.value)} /> </div> <button onClick={handleSaveShop} disabled={submitting} className="h-14 px-8 bg-slate-900 text-white rounded-2xl font-bold hover:bg-black transition-all flex items-center justify-center gap-2 text-sm shadow-xl shadow-slate-200 disabled:opacity-70 disabled:cursor-not-allowed whitespace-nowrap"> {submitting ? <Loader2 size={18} className="animate-spin" /> : (editingShopId ? <Save size={18} /> : <Plus size={18} />)} {editingShopId ? 'Update' : 'Add Shop'} </button> {editingShopId && (<button onClick={() => { setEditingShopId(null); setNewShopName(''); setNewShopRent(''); }} className="h-14 w-14 flex items-center justify-center bg-white border border-slate-200 text-slate-400 rounded-2xl hover:text-rose-500"> <X size={20} /> </button>)} </div> </div>
               </div>
            </section>
                    ...
                    {isCustomExpense ? (
                      ...
                      <div className="relative animate-in fade-in zoom-in duration-200">
                        ...
                        <button 
                          ...
                          className="absolute right-3 top-1/2 -translate-y-1/2 p-2 bg-white text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-xl border border-slate-100 shadow-sm transition-all active:scale-90" 
                      ...
                      <div className="relative group/select">
                        ...
                        <div className="absolute right-5 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400 group-hover/select:text-violet-500 transition-colors"> <ChevronRight size={20} className="rotate-90" /> </div>
                      </div>
                    )}
                  </div>
                </div> 
                ...
                <div className="flex gap-4"> 
                  <div className="w-1/2"> 
                    ...
                    <div className="relative group/amount">
                      <span className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-xl group-focus-within/amount:text-violet-500 transition-colors">₹</span>
                      <input 
                        type="number" 
                        placeholder="0" 
                  ...
                  <div className="w-1/2"> 
                    ...
                    <div className="relative group/payer">
                        ...
                        <div className="absolute right-5 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400 group-hover/payer:text-violet-500 transition-colors"> <UserCircle size={22} /> </div>
                    </div>
                  </div> 
                </div> 
            ...
            <section className="bg-white rounded-[2rem] shadow-xl shadow-slate-200/40 border border-slate-100 overflow-hidden"> 
              <div className="p-6 border-b border-slate-50 flex items-center justify-between"> 
                <h2 className="text-lg font-bold flex items-center gap-3 text-slate-800">
                  <div className="p-2.5 bg-rose-50 rounded-2xl text-rose-500 border border-rose-100"><Receipt size={22} /></div>
                  Expenditures
                </h2> 
                <div className="bg-rose-50 text-rose-600 px-4 py-2 rounded-xl text-xs font-bold tracking-wide border border-rose-100">
                  Total: ₹{formatCurrency(monthlyData.totalExpenses)}
                </div> 
              </div> 
              <div className="p-4 space-y-3 custom-scrollbar"> 
                {expenses.length === 0 ? (<p className="text-center py-10 text-slate-400 text-sm font-medium italic">No expenses recorded for this month.</p>) : (expenses.map((exp) => (
                  <div key={exp.id} className="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-100 shadow-sm gap-3 hover:border-indigo-100 transition-colors"> 
                    <div className="flex items-center gap-4"> 
                      <div className="w-12 h-12 rounded-xl bg-slate-50 text-slate-400 flex items-center justify-center border border-slate-100"><Receipt size={20} /></div> 
                      <div> 
                        <p className="font-bold text-slate-700 text-sm">{exp.description}</p> 
                        <p className="text-[10px] text-slate-400 uppercase tracking-wider font-bold mt-0.5">Paid by <span className="text-indigo-500">{exp.paid_by}</span></p> 
                      </div> 
                    </div> 
                    <div className="flex items-center gap-4"> 
                      <span className="font-bold text-rose-500 text-lg font-mono">-₹{formatCurrency(exp.amount)}</span> 
                      <button onClick={() => deleteExpense(exp.id)} disabled={processingId === exp.id} className="p-2 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-colors"> {processingId === exp.id ? <Loader2 size={18} className="animate-spin text-rose-500" /> : <Trash2 size={18} />} </button> 
                    </div> 
                  </div>
                )))} 
              </div> 
          ...
          <div className="space-y-8">
            <section className="bg-white rounded-[2rem] shadow-xl shadow-slate-200/40 p-6 border border-slate-100">
              <h3 className="text-lg font-bold mb-6 flex items-center gap-3">
                <div className="p-2.5 bg-indigo-50 rounded-2xl text-indigo-600 border border-indigo-100"><UserCircle size={22} /></div>
                Balance Sheet
              </h3>
            ...
            <div className="pb-8">
              <button onClick={handleShare} className="w-full flex items-center justify-center gap-3 px-6 py-5 bg-emerald-600 text-white rounded-3xl font-bold hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-200 text-base active:scale-95"> 
                <Share2 size={22} /> 
